...
def clinical_statistical_clean_data(file_path, output_path, is_diabetes=True):
    ...
    if 'sex' in df.columns:
        df['sex'] = (
            df['sex'].astype(str).str.upper()
            .map({'MALE': 1, 'FEMALE': 0, '1': 1, '0': 0})
            .fillna(0).astype(int)
        )
    ...
    # 🔥 ค่าเฉลี่ย (Mean) และส่วนเบี่ยงเบนมาตรฐาน (Std) ตามหลักสถิติการแพทย์
    ...
    # สร้างประวัติยาตามเกณฑ์ความสมเหตุสมผลทางการแพทย์
...
if __name__ == "__main__":
    ...
    clinical_statistical_clean_data(
        "data/raw/data_dictionary_diabetes.xlsx",
        "data/processed/cleaned_diabetes.xlsx",
        is_diabetes=True,
    )
    clinical_statistical_clean_data(
